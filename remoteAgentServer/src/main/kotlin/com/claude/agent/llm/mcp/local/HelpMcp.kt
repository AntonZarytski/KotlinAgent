...
class HelpMcp(
    ...
    override val tool: Pair<String, LocalToolDefinition> = Pair(
        ...
        second = LocalToolDefinition(
            ...
            description = """
                ...
                ⚠️ ВАЖНО: Используй этот инструмент ТОЛЬКО когда пользователь явно запрашивает помощь по проекту
                или использует команду /help. НЕ используй для решения технических проблем пользователя!

                КОГДА ИСПОЛЬЗОВАТЬ:
                - Пользователь написал "/help" или "/help <тема>"
                - Пользователь явно спрашивает "как работает проект", "покажи архитектуру", "какие есть API"
                - Пользователь просит документацию или примеры кода проекта

                КОГДА НЕ ИСПОЛЬЗОВАТЬ:
                - Пользователь сообщает о проблеме/ошибке (используй support_crm)
                - Пользователь задает вопрос о своем коде (отвечай напрямую)
                - Пользователь просит что-то сделать (используй plan_actions)
    ...
    private fun generateOverview(): String {
        val overview = buildString {
            ...
            appendLine()
            appendLine("KotlinAgent - это AI-агент на базе Claude 3.5 Sonnet с поддержкой MCP (Model Context Protocol).")
            appendLine()
            appendLine("### Основные компоненты:")
            appendLine()
            appendLine("- **Backend**: Ktor server (Kotlin)")
            appendLine("- **Frontend**: Compose for Web (Kotlin/JS)")
            appendLine("- **LLM**: Claude 3.5 Sonnet (Anthropic API)")
            ...
            appendLine()
            appendLine("### Chat API")
            appendLine("- `POST /api/chat` - Отправить сообщение Claude")
