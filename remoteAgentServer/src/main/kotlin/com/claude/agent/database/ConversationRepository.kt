...
class ConversationRepository {
    ...
    /**
     * Находит незавершенные напоминания в сессии по типу задачи.
     * Используется для обновления существующих задач вместо создания дубликатов.
     */
    fun findPendingRemindersBySession(sessionId: String, taskType: String): List<Reminder> {
        return mainDbTransaction {
            Reminders.selectAll().where {
                (Reminders.sessionId eq sessionId) and
                (Reminders.taskType eq taskType) and
                (Reminders.done eq false) and
                (Reminders.notified eq false)
            }.map { row ->
                Reminder(
                    id = row[Reminders.id],
                    sessionId = row[Reminders.sessionId],
    ...
    /**
     * Обновляет task_context существующего напоминания.
     * Используется для добавления дополнительной информации к задаче.
     */
    fun updateReminderContext(id: String, taskContext: String): Boolean {
        return try {
            mainDbTransaction {
                val now = Instant.now().toString()
                Reminders.update({ Reminders.id eq id }) {
                    it[Reminders.taskContext] = taskContext
                    it[updated_at] = now
                }
                logger.info("Обновлен task_context для reminder: $id")
                true
            }
        } catch (e: Exception) {
            logger.error("Ошибка обновления task_context: ${e.message}")
            false
        }
    }

    /**
     * Обновляет текст и due_at существующего напоминания.
     * Используется когда пользователь уточняет время или описание задачи.
     */
    fun updateReminder(id: String, text: String? = null, dueAt: String? = null, taskContext: String? = null): Boolean {
        return try {
            mainDbTransaction {
                val now = Instant.now().toString()
                Reminders.update({ Reminders.id eq id }) {
                    if (text != null) it[Reminders.text] = text
                    if (dueAt != null) it[due_at] = dueAt
                    if (taskContext != null) it[Reminders.taskContext] = taskContext
                    it[updated_at] = now
                }
                logger.info("Обновлен reminder: $id (text=${text != null}, dueAt=${dueAt != null}, context=${taskContext != null})")
                true
            }
        } catch (e: Exception) {
            logger.error("Ошибка обновления reminder: ${e.message}")
            false
        }
    }
