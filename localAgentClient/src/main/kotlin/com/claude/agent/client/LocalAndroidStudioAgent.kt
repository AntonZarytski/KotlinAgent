...
//    private val initialAndroidProjectPath: String? = "/Users/aliaksandramolchan/StudioProjects"
    ...
    private fun setProjectPath(arguments: JsonObject): String {
        ...
        logger.info("   Raw path from arguments: $newPath")

        // Нормализуем путь - преобразуем относительный путь в абсолютный
        val normalizedPath = if (newPath.startsWith("/") || newPath.matches(Regex("^[A-Za-z]:\\\\.+"))) {
            // Уже абсолютный путь (Unix или Windows)
            newPath
        } else {
            // Относительный путь - добавляем базовый путь
            val basePath = initialAndroidProjectPath ?: System.getProperty("user.home")
            val fullPath = File(basePath, newPath).absolutePath
            logger.info("   Converted relative path to absolute: $newPath -> $fullPath")
            fullPath
        }

        logger.info("   Normalized path: $normalizedPath")
        val projectDir = File(normalizedPath)

        logger.debug("   Checking if directory exists: ${projectDir.absolutePath}")
        if (!projectDir.exists()) {
            logger.error("❌ [SET_PATH] Directory does not exist: ${projectDir.absolutePath}")
            return errorJson("Project directory does not exist: ${projectDir.absolutePath}")
        }

        logger.debug("   Checking if path is directory...")
        if (!projectDir.isDirectory) {
            logger.error("❌ [SET_PATH] Path is not a directory: ${projectDir.absolutePath}")
            return errorJson("Path is not a directory: ${projectDir.absolutePath}")
        }

        logger.debug("   Updating project path to: ${projectDir.absolutePath}")
        updateProjectPath(projectDir.absolutePath)
        ...
        logger.info("✅ [SET_PATH] Project path set successfully to: ${projectDir.absolutePath}")
        return result
    }
    ...
    private suspend fun getFileTree(arguments: JsonObject): String = withContext(Dispatchers.IO) {
        logger.info("📂 getFileTree called with arguments: $arguments")
        if (androidProjectPath == null) {
            logger.error("❌ Android project path is not configured")
            return@withContext errorJson("Android project path not configured")
        }
        ...
        try {
            ...
            if (!rootDir.exists() || !rootDir.isDirectory) {
                return@withContext errorJson("Project path is not a valid directory: $androidProjectPath")
            }
            ...
            logger.error("❌ Exception during getFileTree: ${e.message}")
            e.printStackTrace()
