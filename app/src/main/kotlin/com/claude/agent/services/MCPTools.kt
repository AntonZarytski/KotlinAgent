...
class MCPTools(
    ...
    companion object {
         ...
         */
        fun getToolsDefinitions(): List<ToolDefinition> {
            return listOf(
                ToolDefinition(
                    ...
                    input_schema = JsonObject(mapOf(
                        ...
                        "properties" to JsonObject(mapOf(
                            "latitude" to JsonObject(mapOf(
                                ...
                                "description" to JsonPrimitive("Широта местоположения (например, 55.7558 для Москвы). Опционально - если не указано, определяется автоматически.")
                            )),
                            "longitude" to JsonObject(mapOf(
                                "type" to JsonPrimitive("number"),
                                "description" to JsonPrimitive("Долгота местоположения (например, 37.6173 для Москвы). Опционально - если не указано, определяется автоматически.")
                            )),
                            "units" to JsonObject(mapOf(
                                "type" to JsonPrimitive("string"),
                                "enum" to JsonArray(listOf(JsonPrimitive("metric"), JsonPrimitive("imperial"))),
                ...
                ToolDefinition(
                    ...
                    input_schema = JsonObject(mapOf(
                        ...
                        "properties" to JsonObject(mapOf(
                            "latitude" to JsonObject(mapOf(
                                "type" to JsonPrimitive("number"),
                                "description" to JsonPrimitive("Широта местоположения (например, 69.6492 для Мурманска). Опционально - если не указано, определяется автоматически.")
                            )),
                            "longitude" to JsonObject(mapOf(
                                "type" to JsonPrimitive("number"),
                                "description" to JsonPrimitive("Долгота местоположения (например, 30.0450 для Мурманска). Опционально - если не указано, определяется автоматически.")
                            ))
                        )),
                        "required" to JsonArray(emptyList())
                    ))
     ...
     */
    suspend fun callTool(toolName: String, arguments: JsonObject, clientIp: String? = null): String {
        return when (toolName) {
            "get_weather_forecast" -> {
                val location = getLocationFromArguments(arguments, clientIp)
                getWeatherForecast(
    ...
    /**
     * Определяет местоположение из аргументов или автоматически по IP.
     */
    private suspend fun getLocationFromArguments(arguments: JsonObject, clientIp: String?): LocationInfo {
        val latitude = arguments["latitude"]?.jsonPrimitive?.doubleOrNull
        val longitude = arguments["longitude"]?.jsonPrimitive?.doubleOrNull

        // Если координаты указаны явно, используем их
        if (latitude != null && longitude != null) {
            logger.info("Используются координаты из аргументов: lat=$latitude, lon=$longitude")
            return LocationInfo(latitude, longitude, "explicit")
        }

        // Иначе определяем местоположение по IP
        ...
        logger.info("Определено местоположение: ${locationData.city}, ${locationData.country} (${locationData.latitude}, ${locationData.longitude})")
